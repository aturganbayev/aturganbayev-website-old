...
<ul style="margin-top: 0; margin-bottom: 8px;">
  ...
  <li>Сгенерировал схемы касаний для полного покрытия поверхности и для 20 вертикальных полос вокруг конуса (по 10 точек на полосу) с масштабируемым по высоте наклоном ориентации (7°–14°) для зазора инструмента, затем выполнил их через собственный решатель прямой/обратной кинематики UR5 на Python (необходимый, поскольку встроенный в контроллер решатель IK с одним начальным приближением не сходится для точек, разнесённых вокруг конуса), передавая единую программу URScript через порт вторичного клиента робота, пока поток состояния в реальном времени продолжал транслировать TCP-позу.</li>
